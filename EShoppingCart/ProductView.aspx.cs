...
public partial class ProductView : System.Web.UI.Page
{
    ...
    private bool checkProductCat()
    {
        ...
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("Select C.CatName from tblProducts P inner join tblCategory C on C.CatID = P.PCategoryID where PID =" + PID, con);
            SqlDataReader reader =cmd.ExecuteReader();
            reader.Read();
            if(reader.HasRows)
            {
                if (reader["CatName"].ToString() == "Saree" || reader["CatName"].ToString() == "Sarees")
                    saree = true;
            }
            con.Close();
        }
    ...
    private void AddToCartProduction()
    {
        ...
        else
        {
            Int64 PID = Convert.ToInt64(Request.QueryString["PID"]);
            Session["PID"] = PID;
            Response.Redirect("Signin.aspx?PID=" + PID);
        }
    }
